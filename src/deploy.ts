/**
 * Deploy meritveil-learning contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
const initialPrivateState = createCounterPrivateStateFromWalletSeed(SEED);
const initialOwnerCommitment = Counter.pureCircuits.deriveOwnerCommitment(initialPrivateState.secretKey);
...
async function createProviders(walletCtx: WalletContext) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'counter-private-state-v1',
      accountId,
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy meritveil-learning to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      // The private state provider binds the random owner secret to this
      // deployment. Only its derived commitment is passed to the public
      // constructor; retries reuse the same secret and commitment.
      deployed = await deployContract(providers, {
        compiledContract: compiledContract as any,
        args: [initialOwnerCommitment],
