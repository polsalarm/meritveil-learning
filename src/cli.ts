...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║                   meritveil-learning CLI                           ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    const deployed: any = await findDeployedContract(providers, {
      ...
      initialPrivateState: createCounterPrivateStateFromWalletSeed(SEED),
    });
    ...
    while (running) {
      ...
      console.log('  2. Read public counter state');
      ...
      switch (choice.trim()) {
        case '1': {
          console.log('\n  Proving ownership and submitting increment...');
          try {
            const tx = await deployed.callTx.increment();
            console.log('\n  ✅ Counter incremented after proving ownership without revealing the secret.');
        ...
        case '2': {
          console.log('\n  Reading public counter state...');
          try {
            const contractState = await providers.publicDataProvider.queryContractState(deployment.address);
            if (contractState) {
              const ledgerState = Counter.ledger(contractState.data);
              console.log(`\n  📋 Counter: ${ledgerState.counter.toString()}`);
              console.log(`  Owner commitment: ${Buffer.from(ledgerState.ownerCommitment).toString('hex')}\n`);
            } else {
