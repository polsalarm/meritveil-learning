/**
 * End-to-end smoke check for meritveil-learning.
 *
...
async function main() {
  ...
  const ledgerState = Counter.ledger(onChainState.data);
  if (ledgerState.counter < 0n || ledgerState.ownerCommitment.length !== 32) {
    await walletCtx.wallet.stop();
    fail('Decoded ledger does not contain a valid counter and 32-byte owner commitment.');
  }

  console.log(`✅ e2e-check passed`);
  console.log(`   contractAddress: ${deployment.address}`);
  console.log(`   network:         ${network}`);
  console.log(`   counter:         ${ledgerState.counter.toString()}`);
